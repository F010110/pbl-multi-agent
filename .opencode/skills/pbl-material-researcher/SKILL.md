...
- 在返回前，必须检查本文件中的关键输出约束，而不是直接提交第一版结果。
- 至少检查：材料条数、每条材料正文长度、标签稳定性、`angle`/`source_note`/`conflict_with` 是否齐全、冲突对数量是否足够、是否同时包含价值/目标冲突与事实/机制冲突。
- 必须逐条检查每个 `content` 是否大体落在 200 到 400 字之间；如果有明显超出范围或明显过短，应先改写再提交。
- 如果某条材料明显不达标，优先重写该条；如果整组材料过于同质或冲突不足，再整体调整。
- 只有当材料包整体可直接支撑后续讨论时，才能对外输出最终结果。
