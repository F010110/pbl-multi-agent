...
这一分支正在试验“自由讨论中每轮发言后，student、ta、peer_high、peer_low 全员实时刷新状态卡”的工作流。
...
- 在本实验分支中，每轮正文发言落盘后，student、ta、`peer_high`、`peer_low` 都会各自再执行一次“只刷新状态卡”的流程，随后 moderator 再读取四张最新状态卡决定下一位发言者。
...
4. 学生先基于材料给出初始看法。
5. 学生、同伴和助教围绕材料展开自适应讨论；在本实验分支中，每轮先生成一位角色的正文发言，再刷新 student、ta、`peer_high`、`peer_low` 的最新状态卡，主持人随后根据这四张最新状态卡、材料覆盖与问题状态决定下一位发言者。
...
- 由于本分支额外加入了“每轮后全员状态刷新”，自由讨论阶段的调用量和等待时间都会高于主分支。
